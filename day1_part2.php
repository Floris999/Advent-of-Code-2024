...
// Your puzzle answer was 23741109.

$input = file_get_contents('input/input_day1.txt');
...
// $countInRight = array_count_values($right)[2] ?? 0 
// geeft 3 (omdat 2 drie keer in $right voorkomt).
// similarityScore += 2 * 3 → similarityScore = 6.

$similarityScore = 0;
foreach ($left as $leftNumber) {
    $countInRight = array_count_values($right)[$leftNumber] ?? 0;
    $similarityScore += $leftNumber * $countInRight;
}

echo "Total similarity score: " . $similarityScore . "\n";

echo "<script>console.log('Total similarity score: " . $similarityScore . "');</script>";
